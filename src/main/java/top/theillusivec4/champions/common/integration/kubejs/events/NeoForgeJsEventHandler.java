...
public class NeoForgeJsEventHandler {
  ...
  @SubscribeEvent(priority = EventPriority.LOWEST)
  public void onAddSephiahName(SpawnChampionEvent.AddSephiahName event) {

    if (EventHandlers.addSephiahName.hasListeners()) {
      var eventJs = new SpawnChampionEventJs.AddSephiahName(event.getChampion(), event.getName());
      EventHandlers.addSephiahName.post(eventJs);

      event.setName(eventJs.getName());
      // 同步取消 forge event
      if (eventJs.isCanceled()) {
        event.setCanceled(true);
      }
