...
public class ArcticBullet extends ShulkerBullet {
  ...
  public ArcticBullet(Level level, Entity owner, @Nullable Entity target, Direction.Axis invalidStartAxis) {
    this(EntityTypes.ARCTIC_BULLET.get(), level);
    this.setOwner(owner);
    Vec3 position = owner.getBoundingBox().getCenter();
    this.snapTo(position.x, position.y, position.z, this.getYRot(), this.getXRot());
  ...
  public ArcticBullet(EntityType<? extends ShulkerBullet> type, Level level) {
    super(type, level);
  }
