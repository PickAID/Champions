...
public final class SpawnEggCommand {
	...
	private static int rank(CommandSourceStack source, Collection<ServerPlayer> players, Holder.Reference<Rank> rank) {
		...
		source.sendSuccess(() -> Component.translatable(LanguageKeys.COMMANDS_RANK_SUCCESS, rank.value().description()), true);
		return i;
	}
	...
	private static int affix(CommandSourceStack source, Collection<ServerPlayer> players, Holder.Reference<Affix> affix) {
		...
		for (ServerPlayer player : players) {
			ItemStack itemStack = player.getMainHandItem();
			ChampionHelper.updateAffixContainerStored(itemStack, mutable -> mutable.add(affix));
//      ChampionUtil.getHandler(itemStack)
//        .ifPresent(handler -> handler.updateAffixes(mutable -> mutable.add(affix)));
		}
		source.sendSuccess(() -> Component.translatable(LanguageKeys.COMMANDS_AFFIX_SUCCESS, affix.value().description()), true);
		return i;
	}
	...
	private static int level(CommandSourceStack source, Collection<ServerPlayer> players, int level) {
		...
		for (ServerPlayer player : players) {
			ItemStack itemStack = player.getMainHandItem();
			ChampionHelper.setLevel(itemStack, level);
//      ChampionUtil.getHandler(itemStack)
//        .ifPresent(handler -> handler.setLevel(level));
			i++;
		}

		source.sendSuccess(() -> Component.translatable(LanguageKeys.COMMANDS_LEVEL_SUCCESS, level), true);
		return i;
	}
	...
	private static int boss(CommandSourceStack source, Collection<ServerPlayer> players, boolean boss) {
		...
		for (ServerPlayer player : players) {
			ItemStack itemStack = player.getMainHandItem();
			ChampionHelper.setBoss(itemStack, boss);
//      ChampionUtil.getHandler(itemStack)
//        .ifPresent(handler -> handler.setBoss(boss));
			i++;
		}

		source.sendSuccess(() -> Component.translatable(LanguageKeys.COMMANDS_BOSS_SUCCESS, boss), true);
		return i;
	}
	...
	private static int color(CommandSourceStack source, Collection<ServerPlayer> players, int color) {
		...
		for (ServerPlayer player : players) {
			ItemStack itemStack = player.getMainHandItem();
			ChampionHelper.setColor(itemStack, color);
//      ChampionUtil.getHandler(itemStack)
//        .ifPresent(handler -> handler.setColor(ARGB.opaque(color)));
			i++;
		}

		source.sendSuccess(() -> Component.translatable(LanguageKeys.COMMANDS_COLOR_SUCCESS, LanguageUtil.getColorComponent(color)), true);
		return i;
	}
}
